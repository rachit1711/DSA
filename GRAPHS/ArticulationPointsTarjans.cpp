...
void dfs(int node,int parent, vector<int> &disc,vector<int> &low,  unordered_map<int,bool> &vis,unordered_map<int,list<int>> &adj,vector<int> &ap,int timer)
{
    ...
    for(auto nbr:adj[node])
    {
        ...
        if(!vis[nbr])
        {
            dfs(nbr,parent,disc,low,vis,adj,ap,timer);
            //update low[node] on returning back 
        ...
        {
            //nbr is not parent and is visited also it means its a back edge 
            //update the low[node]
            low[node]=min(low[node],disc[nbr]);
        }
    }
    //edge case if the root node has multiple child
    if(parent==-1 && child > 1)
    {
        ap[node]=1;
    }
